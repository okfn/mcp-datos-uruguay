...
def matriz_generacion_electrica(anio_desde=None, anio_hasta=None) -> DataToolOutput:
    ...
    df = c.load_dataset("generacion")
    df = c.filter_years(df, anio_desde, anio_hasta)
    src = [c.DATASET_PAGES["generacion"]]
    if df.empty:
        return c.empty_result("de generación eléctrica en ese rango", src)
    ...
    lines = [
        ...
        "",
        f"Mix {anio_ult} — TOTAL = {c.fmt_num(ult['TOTAL'])} GWh:",
    ] + breakdown_lines
    lines.append(c.unit_blurb("GWh"))
    lines.append("")
...
def potencia_instalada_por_fuente(anio_desde=None, anio_hasta=None) -> DataToolOutput:
    ...
    df = c.load_dataset("potencia")
    df = c.filter_years(df, anio_desde, anio_hasta)
    # Los años pre-2003 vienen vacíos; descartar filas donde TOTAL es NaN.
    df = df[df["TOTAL"].notna()].reset_index(drop=True)
    src = [c.DATASET_PAGES["potencia"]]
    if df.empty:
        return c.empty_result("de potencia instalada en ese rango", src)
    ...
    lines = [
        ...
        "",
        f"Mix de capacidad {anio_ult} — TOTAL = {c.fmt_num(ult['TOTAL'])} MW:",
    ] + breakdown_lines
    lines.append("")
    lines.append(
...
def factor_emision_electrico(anio_desde=None, anio_hasta=None) -> DataToolOutput:
    ...
    df = c.load_dataset("factor_emision_sin")
    df = c.filter_years(df, anio_desde, anio_hasta)
    df = df[df["FE_SIN"].notna()].reset_index(drop=True)
    src = [c.DATASET_PAGES["factor_emision_sin"]]
    if df.empty:
    ...
    lines = [
        ...
        f"(emisiones {c.fmt_num(em_ult, 1)} Gg CO2 sobre "
        f"{c.fmt_num(ee_ult)} GWh entregados).",
    ...
    lines.append(c.unit_blurb("t CO2/GWh", "Gg CO2", "GWh"))
    lines.append("")
    lines.append(c.SOURCE_FOOTER)
    ...
    for _, row in df.iterrows():
        table.append([
            ...
            c.fmt_num(row["Em_CE_SP"], 1),
            c.fmt_num(row["EE_CE_SP"]),
        ])
